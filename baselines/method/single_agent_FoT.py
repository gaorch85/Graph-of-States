...
class SingleAgentFoT(BaseMethod):
    ...
    def run(self) -> Dict[str, str]:
        ...
        for _i in range(N):
            ...
            log_list.append(detailed_log)
            final_answer_list.append(final_answer)
            final_report_list.append(final_report)
        
        for idx, log in enumerate(log_list):
            self.write_log(f"SingleAgentFoT_Sample{idx+1}", "SingleAgentFoT", self.data)
            self.write_log(log, "SingleAgentFoT", self.data)
            print(f"Detailed log for sample {idx+1} saved successfully!")
        
        final_answer, final_report = self.aggregate(final_answer_list, final_report_list)
        self.write_log( f"Final Aggregated Answer: {final_answer}\n\nFinal Aggregated Report: {final_report}\n\n\n\n[Individual Sample Logs]\n","SingleAgentFoT",self.data)
